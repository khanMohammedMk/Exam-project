...
// //$n=$_POST['nr'];
//   //$i=1;
$query = "select count(COLUMN_NAME) as am from INFORMATION_SCHEMA.Columns where TABLE_NAME = 'ROOM$r'";
// echo $query;
$col = mysqli_query($conn,$query) or die(mysqli_error($conn));
$result = mysqli_fetch_array($col);
$no_of_col = $result['am'];
// echo $no_of_col;
...
echo "<br>Available Seat:.<br>";
$total_seat = 0;
for($i=1;$i<$no_of_col;$i=$i+1){
	$quer = "SELECT COUNT(*) AS sea FROM ROOM$r WHERE col$i IS NULL";
	$res = mysqli_query($conn,$quer) or die(mysqli_error($conn));
	$output = mysqli_fetch_array($res);
	$in = $output['sea'];
	$total_seat = $total_seat + $in;
}
echo $total_seat;
